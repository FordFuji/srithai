...
class Backend extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('banner/model_banner');
		$this->load->model('banner/model_banner_datatable');
	...
	public function server_processing() {
        ...
        $results = $this->model_banner_datatable->banner_datatable($param);
	...
	public function form($id = ''){	
		$data['id'] = $id;
		$data['row'] = $this->model_banner->get_data_single($id);
		...
		$this->load->view('banner/banner/form', $data);
	}
	...
	public function save_update($id = ''){	
		...
		if(!empty($_FILES['banner_image'])) {
			...
			if($this->upload->do_upload('banner_image')) {
				...
				$data['banner_image'] = $data_image['file_name'];
			} else {
		...
		// update 
		if($id != '') {	
			$this->model_banner->update_data($data, $id);
			
			redirect('banner/backend/index', 'location');
			
		// insert
		} else {	
			$data['banner_datetime_create'] = date('Y-m-d H:i:s');
			$data['banner_ip_create'] = $_SERVER['REMOTE_ADDR'];
				
			$this->model_banner->insert_data($data);
	...
	public function delete($id){
		$this->model_banner->delete_data($id);
