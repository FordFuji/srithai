...
class Path extends CI_Controller {
	...
	public function index() {
		...
		$data['popup'] = $this->model_frontend->getPopupRecord();
	...
	public function article_detail($article_id) {
		...
		$data['banner_article'] = $this->model_frontend->getBannerArticle();
	...
	public function confirm_payment($order_id = '') {
		...
		$data['bank'] = $this->model_frontend->getBankResult();
	...
	public function member_order() {
		$data['order'] = $this->model_frontend->getOrderResult__();
	...
	public function success($order_no) {
		...
		if(!empty($row)) {
			...
			if(!empty($row)) {
				...
				$sender[] = 'E-com@srithaisuperware.com';
...
<?php
	...
	public function ajaxCheckout() {
		...
		if(!empty($this->input->post('order_address_for_billing'))) {
			$data_sess[] = $this->input->post('order_address_for_billing');
		}
		...
		if($this->input->post('payment_method') == 'Bank Transfer') {
			...
			if($this->input->post('shipping') == 'default') {
				...
				if($this->input->post('switch') == 'Yes') {
					...
					$data['order_address_for_billing'] = $this->input->post('order_address_billing');
			...
			$this->db->order_by('order_id', 'desc');
			$query_order = $this->db->get('ci_order');
	...
	public function ajaxUpdateCart() {
		...
		$this->model_frontend->getUpdateBuyAndGiveAway();
	...
	public function ajaxCart() {
		...
		$province_id = $this->session->userdata('order_province');
		
		$shipping_price = 0;
		if(!empty($province_id)) {
			$shipping_price = $this->model_frontend->getShippingPrice($weight, $province_id);
		} else {
			$province = $this->model_frontend->get_shipping_address_status_main();

			if(!empty($province)) {
				$province_id = $province->member_shipping_province;
		...
		$data_sess = array(
			...
			'order_shipping_text' => $shipping_price
		);

		$this->session->set_userdata($data_sess);

		if($this->session->userdata('check_shipping') == 'express') {
			$data_sess = array(
				'order_shipping' => $shipping_price,
				'order_shipping_text' => $shipping_price
			);
			...
			$this->session->set_userdata($data_sess);
		} elseif($this->session->userdata('check_shipping') == 'me') {
			$data_sess = array(
				'order_shipping' => 0,
				'order_shipping_text' => $shipping_price
			);
...
<?php
	...
	public function ajaxClickNewAddress() {
		$this->db->where('member_shipping_id', $this->input->post('member_shipping_id'));
		$query = $this->db->get('ci_member_shipping');
		...
		if(!empty($row)) {
			...
			$this->session->set_userdata($data_shipping_address);

			$this->ajaxCart();
		}
	}

	public function ajaxShippingPriceOnLoad() {
		$this->db->where('member_id', $this->session->userdata('member_id'));
	...
	public function ajaxCheckShipping() {
		if($this->input->post('shipping_type') == 'express') {
			$data_sess = array(
				'check_shipping' => 'express'
			);

			$this->session->set_userdata($data_sess);
		} elseif($this->input->post('shipping_type') == 'me') {
			$data_sess = array(
				'check_shipping' => 'me'
			);
		...
		pre($this->session->userdata('check_shipping'));

		$this->ajaxCart();
	}

	public function testPhpMailer123() {
		$this->load->helper('phpmailer');

		$sender = array(
			'sitiporn@orange-thailand.com',
			'nirvanaford94@gmail.com'
		);
