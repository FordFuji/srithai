...
class Model_frontend extends CI_Model {
	...
	public function getBannerArticle() {
		$this->db->where('banner_article_id', 1);
	...
	public function get_category2_result_($category1_id) {
		...
		$this->db->where('ci_product.product_enable', 'Enable');
		$query = $this->db->get('ci_category2');
	...
	public function getOrderResult__() {
		$this->db->select('*, ci_order.order_no as order_no_');
		$this->db->order_by('ci_order.order_id', 'desc');
		$this->db->where('ci_order.member_id', $this->session->userdata('member_id'));
		$this->db->join('ci_payment', 'ci_order.order_no = ci_payment.order_no', 'left outer');
		$query = $this->db->get('ci_order');
	...
	// public function getOrderRecord_($order_id) {
	// 	$this->db->order_by('ci_payment.payment_id', 'desc');
	// 	$this->db->where('ci_order.order_id', $order_id);
	// 	$this->db->join('ci_payment', 'ci_order.order_no = ci_payment.order_no', 'inner');
	// 	$query = $this->db->get('ci_order');
	...
	public function getShippingPrice($weight, $province_id) {
		if($province_id == 1 or $province_id == 2 or $province_id == 3 or $province_id == 4) {
				...
				if($buy_no >= $r->buy_no) {
					...
					if(!empty($giveaway) and $price_buy_and_giveaway == false) {
						foreach($giveaway as $g) {
							$this->db->where('product_id', $g->product_id);
							$query = $this->db->get('ci_product');

							$qty_min_ = $query->row();

							if(!empty($qty_min_)) {
								if($qty_min > $qty_min_->product_stock) {
									$qty_min = $qty_min_->product_stock;
								}
	...
	public function getUpdateBuyAndGiveAway() {
					...
					if(!empty($giveaway)) {
						foreach($giveaway as $g) {
							...
							if(!empty($qty_min_)) {
								...
								if($qty_min_->product_stock < $qty_min) {
									$qty_min = $qty_min_->product_stock;
								}
	...
	public function getQtyByOrder($order_id) {
		$this->db->where('order_id', $order_id);
		$query = $this->db->get('ci_order_detail');
	...
	public function getLinkSocialResult() {
		$this->db->where('link_social_id', 1);
