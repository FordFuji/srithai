...
function dateTime2TextEn($datetime) {
	if(!empty($datetime)) {
		$time_exp = explode(' ', $datetime);

		$time_ = explode(':', $time_exp[1]);

		$exp = explode('-', $time_exp[0]);
		...
		return $day_.' '.$month_.' '.$year_.' '.$time_[0].':'.$time_[1];
	}
}
...
function date2dateNormal($datetime) {
