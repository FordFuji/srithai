...
class Backend extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('point_per_baht/model_point_per_baht');
		$this->load->model('template_main/model_template_main');
		//$this->load->library('datatables');
		$this->load->library('table');
		$this->load->helper('form');
		$this->path_upload = FCPATH.'uploads/point_per_baht/';
	...
	public function index() {
		$data['row'] = $this->model_point_per_baht->get_data_single();
		...
		/* start body */
		$this->load->view('point_per_baht/point_per_baht/form', $data);
		/* end body */
	}
	...
	/*public function server_processing() {
        ...
        $results = $this->model_point_per_baht_datatable->payment_datatable($param);
	...
	/*public function form($id = ''){	
		$data['id'] = $id;
		$data['row'] = $this->model_point_per_baht->get_data_single($id);
	...
	public function save_update($id = ''){	
		$data = array(
			'point_per_baht_amount' => $this->input->post('point_per_baht_amount'),
			'point_per_baht_datetime_update' => date('Y-m-d H:i:s')
		);
		...
		// update 
		$this->model_point_per_baht->update_data($data, 1);
		
		redirect('point_per_baht/backend/index', 'location');
	}
	
	/*public function delete($id){
		$this->model_point_per_baht->delete_data($id);
